    ...
    def __call__(
        ...
        **kwargs,
        ...
        for t in self.progress_bar(self.scheduler.timesteps):
            ...
            model_output = self.unet(image, t.to(self.my_device), class_labels=condition.float()).sample
            if cfg_scale is not None: # classifier free guidance
                uncond_output = self.unet(image, t.to(self.my_device), class_labels=None).sample
...
class MyConditionedUNet(ModelMixin, ConfigMixin):
    ...
    def __init__(
        ...
        class_embed_type: str = "multi_labels",
        freq_shift: int = 0,
        ...
        if class_embed_type is None and num_class_embeds is not None:
            ...
            self.class_embedding = nn.Linear(num_class_embeds, time_embed_dim)        # time_embed_dim = 512
        else:
