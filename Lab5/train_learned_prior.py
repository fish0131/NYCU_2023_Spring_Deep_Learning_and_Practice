...
from utils import init_weights, kl_criterion_lp, plot_pred, plot_rec, finn_eval_seq, pred
...
def parse_args():
    ...
    parser.add_argument('--tfr_decay_step', type=float, default=0.01666, help='The decay step size of teacher forcing ratio (0 ~ 1)')
...
if __name__ == '__main__':
    ...
    if args.model_dir != '':
        ...
        print(f'Using pre-trained: {args.model_dir}/99_model.pth')
    else:
        name = 'kl_cyclical=%s-rnn_size=%d-predictor-posterior-rnn_layers=%d-%d-n_past=%d-n_future=%d-lr=%.4f-g_dim=%d-z_dim=%d-last_frame_skip=%s-beta=%.7f-test02'\
            % (args.kl_anneal_cyclical, args.rnn_size, args.predictor_rnn_layers, args.posterior_rnn_layers, args.n_past, args.n_future, args.lr, args.g_dim, args.z_dim, args.last_frame_skip, args.beta)
    ...
    headerList = ['Epoch', 'KL Weight', 'TFR', 'Loss', 'MSE Loss', 'KLD', 'PSNR']
    ...
    for epoch in range(start_epoch, start_epoch + niter):
        ...
        for i, (seq, cond) in enumerate(tqdm(train_loader)):
            ...
            loss, mse, kld = train(seq, cond, modules, criterion, optimizer, kl_anneal, args)
            epoch_loss += loss
            epoch_mse += mse
            epoch_kld += kld
        ...
        # save KL weights
        epoch_plotting_data.append(kl_anneal.get_beta())

        # save teacher forcing ratio
        epoch_plotting_data.append(tfr_value)

        # save loss
        epoch_plotting_data.append(epoch_loss / args.epoch_size)
        epoch_plotting_data.append(epoch_mse / args.epoch_size)
        epoch_plotting_data.append(epoch_kld / args.epoch_size)
