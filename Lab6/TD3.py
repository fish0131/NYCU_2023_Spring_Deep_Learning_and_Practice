...
class CriticNet(nn.Module):
    ...
    def forward(self, x, action):
        q1 = self.critic_head(torch.cat([x, action], dim=1))
        q1 = self.critic(q1)
...
class TD3:
    ...
    def _update_behavior_network(self, gamma):
        ...
        # critic loss
        ## TODO ##
        q1_current, q2_current = critic_net(state, action)
        with torch.no_grad():
            #exploration_noise = torch.tensor(self._action_noise.sample(), device=self.device)
            exploration_noise = (torch.randn_like(action) * self.policy_noise).clamp(-self.noise_clip, self.noise_clip)
            a_next = (target_actor_net(next_state) + exploration_noise).clamp(-self.max_action, self.max_action)
            # a_next = target_actor_net(next_state)
            #print(next_state.shape)
            #print(a_next.shape)
            q1_target, q2_target = target_critic_net(next_state, a_next)
            ...
            # if self.env_with_dw:
            #     q_target = reward + gamma * q_target * (1 - done)  #dw: die or win
            # else:
            #     q_target = reward + gamma * q_target

        criterion = nn.MSELoss()
        critic_loss = criterion(q1_current, q_target) + criterion(q2_current, q_target)
