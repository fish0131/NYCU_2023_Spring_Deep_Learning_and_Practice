...
private:
	 ...
	 */
	 ...
	 // Optimizing the game's AI by precomputing the results of all possible moves, their scores and board configurations
	struct lookup {
		...
		void init(int r) {
			...
			int L[4] = { V[0], V[1], V[2], V[3] }; // L array is a copy of the 4 tiles of the row in left-to-right order
			int R[4] = { V[3], V[2], V[1], V[0] }; // mirrored: R array is a copy of the 4 tiles of the row in right-to-left order

			score = mvleft(L);
			left = ((L[0] << 0) | (L[1] << 4) | (L[2] << 8) | (L[3] << 12)); // set to a 16-bit integer representation of the new row after the left move

			score = mvleft(R); std::reverse(R, R + 4); // std::reverse: reverse the array after the merge
			right = ((R[0] << 0) | (R[1] << 4) | (R[2] << 8) | (R[3] << 12)); // set to the 16-bit integer representation of the new row after the right move
		}

		// Modify the given raw board row and add the appropriate score for the corresponding move
		...
		// Implements the game's merge rules by merging adjacent tiles of the same value and shifting them to the left
		static int mvleft(int row[]) {
			int top = 0; // keep tarck of the current index of the row where the next tile will be placed
			int tmp = 0; // keep tarck of the value of the previous non-zero tile
			int score = 0;
		...
		static const lookup& find(int row) {
			static const lookup cache[65536]; // 65536 is the total number of possible 16-bit rows
			return cache[row];
		}
	};
...
public:
	 ...
	 */
	void popup() {
		int space[16], num = 0; // num: the number of empty cell
		for (int i = 0; i < 16; i++)
			if (at(i) == 0) {
				space[num++] = i;
			}
		// if there is at least one empty cell on the game board (if num>0)
		if (num)
			/* rand() % 10 generates a random number between 0 and 9. 
			 * If this number is not equal to 0 (i.e. if it's between 1 and 9), then the tile will be set to 1. 
			 * Otherwise, if the random number is equal to 0, the tile will be set to 2.
			 */
	...
	int move_left() {
		uint64_t move = 0;
		uint64_t prev = raw; // the current state of the grid
		int score = 0;
		...
		raw = move;
		return (move != prev) ? score : -1; // If the state of the grid has changed, return score, otherwise return -1
	}
	int move_right() {
		uint64_t move = 0;
		uint64_t prev = raw; // the current state of the grid
	...
	int move_up() {
		rotate_right(); // rotates the board to the right (clockwise)
		int score = move_right();
		rotate_left(); // the board is rotated back to its original orientation
		return score;
	}
	int move_down() {
		rotate_right();
...
public:
	...
	// enable output streaming of a feature object to an output stream
	...
	// enable input streaming of a feature object from an input stream
...
public:
	pattern(const std::vector<int>& p, int iso = 8) : feature(1 << (p.size() * 4)), iso_last(iso) {
		...
		// Why there are 8 isomorphisms?
		// 4 possible 90-degree rotations (0, 1, 2, 3)
		// each of these rotations, there are two possible mirror reflections (horizontal and vertical)
		// 4 * 2 = 8
		for (int i = 0; i < 8; i++) {
...
public:
	...
	virtual float estimate(const board& b) const {
		// TODO
		float value = 0;
		// calculating the pattern index for each of the 8 isomorphic boards of 'b', and then adding the corresponding weights together
        for (int i = 0; i < iso_last; i++) {
            size_t index = indexof(isomorphic[i], b);
            value += operator[](index); // the sum of the weights for all isomorphic patterns and the input board
        }
        return value;
	}
	 ...
	 * update the value of a given board, and return its updated value
	 */
	virtual float update(const board& b, float u) { // 'u' is the update value to be applied to the feature weights
		// TODO
		float u_split = u / iso_last; // this ensures that the update value is distributed equally among all the isomorphic patterns
        float value = 0;
        for (int i = 0; i < iso_last; i++) {
            size_t index = indexof(isomorphic[i], b);
...
protected:
	...
	size_t indexof(const std::vector<int>& patt, const board& b) const {
        ...
        for (size_t i = 0; i < patt.size(); i++)
            index |= b.at(patt[i]) << (4 * i); // setting the corresponding 4-bit nibble in the index to the value at that position in the board
        return index;
	}

	// converting each integer in the vector to its hexadecimal representation
...
public:
	...
	void add_feature(feature* feat) {
		...
		size_t usage = feat->size() * sizeof(float); // the feature size
		if (usage >= (1 << 30)) {
			info << " (" << (usage >> 30) << "GB)";
		} else if (usage >= (1 << 20)) {
			info << " (" << (usage >> 20) << "MB)";
	...
	state select_best_move(const board& b) const {
        ...
        for (state* move = after; move != after + 4; move++) {
			...
			// If the move is valid, it calculates the expected value of the move by considering each possible new tile
            if (move->assign(b)) {
                // TODO
                int num = 0; // keeps track of the number of empty cells
                float value = 0;
                board tmp;
                for (int i = 0; i < 16; i++) {
                    if (move->after_state().at(i) == 0) {
						// the temporary board with the current empty cell filled with a 1 (for a probability of 0.9)
                        tmp = board(move->after_state()); tmp.set(i, 1);
                        value += 0.9 * estimate(tmp);
						// another temporary board with the current empty cell filled with a 2 (for a probability of 0.1)
                        tmp = board(move->after_state()); tmp.set(i, 2);
                        value += 0.1 * estimate(tmp);
                        num++;
                    }
                }
                move->set_value(move->reward() + value / num); // value/num: the expected value

				// If the value of the current state is greater than the value of the best state, 
				// the best pointer is updated to point to the current state
                if (move->value() > best->value()) {
                    best = move;
                }
            }
			// If the move is invalid, the value of the state is set to negative infinity, indicating that this move should not be selected
			else {
                move->set_value(-std::numeric_limits<float>::max());
            }
            debug << "test " << *move;
        }
        return *best; // the best pointer represents the move with the highest expected value
	}
	...
	void update_episode(std::vector<state>& path, float alpha = 0.1) const {  // alpha is learning rate
        ...
        for (path.pop_back() /* terminal state */; path.size(); path.pop_back()) {
			...
			// move.reward(): the immediate reward obtained from the environment after taking the last action
			// exact: the sum of rewards obtained from the environment for all the states that have been visited so far in the episode
			// estimate(move.before_state()): the estimate of the value function for the previous state
